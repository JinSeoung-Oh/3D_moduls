...
def makeBEVMap(PointCloud_, boundary):
    ...
    print('chchchchch', PointCloud_top[:, 3])
    ...
    for j in range(len(detections)):
        ...
        if len(detections[j]) > 0:
            for det in detections:
                ...
                drawpoint(img, _x,_y)
                #drawpoint(img, '304', '0')
                #drawpoint(img, '263', '214')
                #drawpoint(img, '304', '214')
                #drawpoint(img, '345', '214')
    ...
    if bev_type == 'kitti':
        ...
        for i in range(len(lidar_paths)):
            ...
            if len(line_sets) == 9:
                ...
                vis.add_geometry(line_set4)
                vis.add_geometry(line_set5)
                vis_add_geometry(line_set6)
                vis.add_geometry(line_set7)
                vis.add_geometry(line_set8)
                vis.add_geometry(line_set9)
                vis.get_render_option().point_size = 1.5 
                vis.run()
                vis.capture_screen_image(save_path, True)
                vis.destroy_window() 
