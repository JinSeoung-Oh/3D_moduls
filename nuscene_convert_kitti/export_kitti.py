...
class KittiConverter:
    def __init__(self,
        ...
        self.nusc = NuScenes(version=nusc_version, dataroot='./data')
        
    def make_other_view_data(self, cam_type, sample_token,cs_record_lid, kitti_to_nu_lidar):
        sample = self.nusc.get('sample', sample_token)
        cam_view_token = sample['data'][cam_type]
        cs_record_lid = cs_record_lid
    
        sd_record_cam = self.nusc.get('sample_data', cam_view_token)
        ...
        #print(velo_to_cam_rot)
        #print(velo_to_cam_trans)
        ...
        dst_im_path = os.path.join(self.nusc_kitti_dir, self.split, 'image', cam_type, sample_token + '.png')
        print(dst_im_path)
        ...
        calib_path = os.path.join(self.nusc_kitti_dir, self.split, 'calib', cam_type, sample_token + '.txt')
        print(calib_path)
    ...
    def nuscenes_gt_to_kitti(self) -> None:
        ...
        for cam in cam_list:
            cam_folder = os.path.join(image_folder, cam)
            cam_calib_folder = os.path.join(calib_folder, cam)
            cam_label_folder = os.path.join(label_folder, cam)
            for folder in [cam_folder, cam_calib_folder, cam_label_folder]:
                if not os.path.isdir(folder):
                    os.makedirs(folder)
        ...
        sample_tokens = []
        for scence in my_scene:
            first = scence['first_sample_token']
            sample_tokens.append(first)     
        ...
        for sample_token in sample_tokens:
            ...
            for cam_type in cam_list:
                print(cam_type)
                self.make_other_view_data(cam_type, sample_token,cs_record_lid, kitti_to_nu_lidar) 
            ...
            src_im_path = os.path.join(self.nusc.dataroot, filename_cam_full)
            dst_im_path = os.path.join(image_folder,'CAM_FRONT', sample_token + '.png')
            if not os.path.exists(dst_im_path):
                im = Image.open(src_im_path)
                im.save(dst_im_path, "PNG")
            ...
            calib_path = os.path.join(calib_folder, 'CAM_FRONT', sample_token + '.txt')
            with open(calib_path, "w") as calib_file:
                for (key, val) in kitti_transforms.items():
                    val = val.flatten()
                    val_str = '%.12e' % val[0]
